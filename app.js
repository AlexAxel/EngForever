...
function next(){if(busy)return;const valid=new Set(allActive().map(p=>p.id));queue=queue.filter(id=>valid.has(id)&&id!==current?.id);if(queue.length){current=phrases.find(p=>p.id===queue.shift());showCard();updateStatus();save();return;}
current=null;const active=allActive();if(active.length){freshQueue();if(queue.length){current=phrases.find(p=>p.id===queue.shift());showCard();updateStatus();save();return;}}
const difficult=phrases.filter(p=>{const c=cardState(p.id);return c.learned&&!c.manuallyDisabled&&c.hardness>=15;});if(difficult.length&&started){round++;for(const p of difficult){const c=cardState(p.id);c.enabled=true;c.learned=false;}queue=shuffle(difficult.map(p=>p.id));current=phrases.find(p=>p.id===queue.shift());showCard();toast("Новый круг: "+difficult.length+" сложных фраз");updateStatus();save();return;}
